...
Early release. Core: explicit targets, run (command / inline / script), copy, per-run secrets (file + env forms), env: convenience step, dependency graph + cycle detection + reachable ordering. Multi-file orchestration (`builds:` + `component:target`), platform overrides, and intra-build parallel execution are planned (grammar/graph prep exists for forward-compat).
