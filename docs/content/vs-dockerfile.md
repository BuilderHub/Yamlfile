...
# Why yamlfile instead of a plain Dockerfile?
...
- `from:` and `copy.from:` (including the `component:target` form for multi-file) declare dependencies.
...
See the [Syntax Reference](/syntax-reference) and the source in `pkg/convert/graph.go` for how the dependency graph is built.
